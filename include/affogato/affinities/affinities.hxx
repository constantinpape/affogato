...
namespace affinities {
    ...
    void compute_affinities(const xt::xexpression<LABELS> & labels_exp,
                            ...
                            const xt::xexpression<MASK> & boundary_mask_exp,
                            const xt::xexpression<MASK> & glia_mask_exp,
        ...
        auto & boundary_mask = boundary_mask_exp.derived_cast();
        auto & glia_mask = glia_mask_exp.derived_cast();
        ...
        {
            const auto shp = aff_shape[d+1];
            if ((shp != labels.shape()[d]) || (shp != boundary_mask.shape()[d]) || (shp != glia_mask.shape()[d]) ) {
                std::cout << shp << "\n";
                std::cout << "Labels: " << labels.shape()[d] << "\n";
                std::cout << mask.shape()[d] << "\n";
                std::cout << boundary_mask.shape()[d] << "\n";
                std::cout << glia_mask.shape()[d] << "\n";
                throw std::runtime_error("Shapes of the current labels, masks and affinities do not agree.");
            }
        }
        ...
        util::for_each_coordinate(aff_shape, [&](const xt::xindex & aff_coord){
            ...
            if(glia_mask[coord] && glia_mask[ngb_coord]) {
                mask[aff_coord] = 0;
                return;
            }
            if(boundary_mask[coord] || boundary_mask[ngb_coord]) {
