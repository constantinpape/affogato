...
def mws_segmentation_kruskal(affs):
    t0 = time.time()
    seperating_channel = 2
...
def mws_segmentation_prim(affs):
    t0 = time.time()
    seperating_channel = 2
    seg = compute_mws_prim_segmentation(seperating_channel,
                                        OFFSETS, affs)
    return seg, time.time() - t0
...
if __name__ == '__main__':
    ...
    seg0, t0 = mws_segmentation_kruskal(affs)
    print("... in %f s" % t0)
    segs = [seg0]
    labels = ['raw', 'seg_mws']

    print("Computing Prim MWS segmentation ...")
    seg1, t1 = mws_segmentation_prim(affs)
    print("... in %f s" % t1)
    segs.append(seg1)
